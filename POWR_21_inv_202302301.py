...
ratio_ema_long_1 = 1.0   # 현재 구간에서의 ema_long값과 직전구간 ema_long값의 비율이 얼마 이상일때 buy transaction을 수행하는가 (여러 필요 조건중의 하나)
...
while True:
    ...
    if (now.minute % time_unit == 1) & (0 < (now.second % 60) <= 15) :   # N시:01:00초 ~ N시:01:15초 사이 시각이면
        ...
        if bought_state == 0 :   # 매수가 없는 상태라면
            ...
            print ('\n[BUY condition check]')
            print ('DF_test_ma[ema_o_l_consecutive_rise][-(ema_long_duration + 1) : -1].sum()_____[condition] greater {0} / [value] {1}'.format(5, DF_test_ma['ema_o_l_consecutive_rise'][-(ema_long_duration + 1) : -1].sum()))
            ...
            print ('DF_test_ma[ema_open_long][-1] / DF_test_ma[ema_open_long][-2])_____[condition] greater than {0} / [value] {1}'.format(ratio_ema_long_1, (DF_test_ma['ema_open_long'][-1] / DF_test_ma['ema_open_long'][-2])))
            ...
            if ((DF_test_ma['ema_o_l_consecutive_rise'][-(ema_long_duration + 1) : -1].sum() > 5) and                 (DF_test_ma['ema_o_s_consecutive_rise'][-(ma_short_under_duration + 1) : -1].sum() > (ma_short_under_duration - 2)) and                 ((DF_test_ma['ema_open_long'][-1] / DF_test_ma['ema_open_long'][-2]) > ratio_ema_long_1) and                 ((DF_test_ma['ratio_vol_to_aver'][-3] > buy_cri_vol_times) or (DF_test_ma['ratio_vol_to_aver'][-2] > buy_cri_vol_times)) and                 (DF_test_ma['ema_open_short'][-1] / DF_test_ma['ema_open_long'][-1] > ratio_ema_o_short_long_buy_1[0]) and (DF_test_ma['ema_open_short'][-1] / DF_test_ma['ema_open_long'][-1] < ratio_ema_o_short_long_buy_1[1])) :               
